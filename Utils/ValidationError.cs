...
public static class ValidationError
{
    ...
    public static string IsExist<T>(T instance, string propertyName)
    {
        ...
        // 返回本地化字符串。外层 key 是带参模板（规范化为 Arg0_Arg1_IsExist），
        // 内层 App.L.R(displayName) 是动态 key（来自 [Display(Name)] 反射），保留字符串重载。
        return App.L.R("Arg0_Arg1_IsExist", App.L.R(displayName), value);
    }
    ...
    {
        // 返回本地化字符串
        return App.L.R("Error_DataNotExist");
    }
    ...
    public static string NotExist<T>(T instance, string displayName, string propertyName)
    {
        ...
        // 返回本地化字符串（外层带参模板，内层动态 key）
        return App.L.R("Arg0_Arg1_NotExist", App.L.R(displayName), value);
    }
    ...
    {
        // 返回本地化字符串
        return App.L.R("Error_DataAssociationExists");
    }
    ...
    public static string Required<T>(T instance, string propertyName)
    {
        ...
        // 返回本地化字符串（外层带参模板，内层动态 key）
        return App.L.R("Arg0_required", App.L.R(displayName));
    }
}
